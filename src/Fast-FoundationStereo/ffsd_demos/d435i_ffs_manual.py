"""
RealSense D435i + Fast-FoundationStereo Manual Capture Depth Estimation and Denoised Color Point Cloud

IR left/right stereo pair → FFS inference → depth map → RGB-colored point cloud → Denoise

Features:
  - Press SPACE or 'c' to capture a frame and process the point cloud.
  - Press ESC or 'q' to exit.
  - Voxel down-sampling and radius outlier removal for denoising.
"""
...
IR_PROJECTOR_ON = True # Enable IR projector
IMG_WIDTH = 640
IMG_HEIGHT = 480
...
# Try loading from Fast-FoundationStereo if weights not found in Physics folder
...
vis.create_window("D435i + FFS Denoised Point Cloud", width=1280, height=720)
    ...
    while True:
        ...
        if key == 27 or key == ord('q'):
            ...
            if len(points_final) == 0:
                logging.warning("No valid points generated! Try moving closer to objects or checking projector.")
                continue
